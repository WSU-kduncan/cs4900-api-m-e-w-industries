...
public class GameController {
  ...
  public ResponseEntity<List<GameDto>> getAllGames() throws EntityNotFoundException {
    ...
    List<GameDto> dtos =
        gameService.getAllGames().stream().map(GameDto::fromEntity).toList();

    return ResponseEntity.ok(dtos);
  }

  @GetMapping("/id/{gameId}")
  public ResponseEntity<GameDto> getGameById(@PathVariable("gameId") Integer gameId)
  ...
  public ResponseEntity<GameDto> getGameByTitle(@PathVariable("gameTitle") String gameTitle)
      throws EntityNotFoundException {
