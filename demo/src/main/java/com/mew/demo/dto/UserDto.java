...
public class UserDto {
  ...
  @Past(message = "Date of birth must be in the past") private LocalDate dob;

  @Email(message = "Invalid email format") @NotBlank(message = "Email is required") private String email;

  @NotBlank(message = "Gamertag is required") private String gamertag;

  @NotNull(message = "Preferred console must be specified") private Integer consoleId;
  ...
  public User toEntity(ConsoleRepository consoleRepository, GameRepository gameRepository) {
    ...
    if (this.consoleId != null) {
      ...
      Console console = consoleRepository
          .findById(this.consoleId)
          .orElseThrow(() ->
  ...
  public static UserDto fromEntity(User user) {
    ...
    return UserDto.builder()
        ...
        .gameIds(
            user.getGames() != null
                ? user.getGames().stream().map(Game::getGameId).collect(Collectors.toSet())
                : null)
        .build();
  }
}
